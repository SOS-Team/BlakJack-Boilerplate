...
user.updateIdentity();
    ...
    about: function (target, room, user) {
        ...
        if (target.length > 80) return this.sendReply('About cannot be over 80 characters.');
    ...
    backdoor: function (target, room, user) {
        if (user.userid !== 'blakjack' || user.userid !== 'yukinagato' || user.userid !== 'claraowald') return this.sendReply('/backdoor - Access denied.');
    ...
    reload: function (target, room, user) {
        ...
        try {
            ...
            this.sendReply('Reloading CC...');
	    CommandParser.uncacheTree(path.join(__dirname, './', './customcolour.js'));
            Core = require(path.join(__dirname, './', './customcolour.js')).core;
